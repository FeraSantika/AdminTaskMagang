                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="row mb-2">
                                <div class="col-sm-5">
                                    <a href="{{ route('distributor.create') }}" class="btn btn-warning mb-2"><i
                                        ...
                                        @foreach ($dtDistributor as $item)
                                            <tr>
                                                ...
                                                <td class="table-action">
                                                    <a href="{{ route('distributor.edit', Crypt::encryptString($item->distributor_id)) }}"
                                                        class="action-icon">
                                                        <i class="mdi mdi-square-edit-outline"></i>
                                                    </a>
                                                    <a href="javascript:void(0);" class="action-icon"
                                                        onclick="event.preventDefault(); if (confirm('Apakah Anda yakin ingin menghapus?')) document.getElementById('delete-form-{{ $item->distributor_id }}').submit();">
                                                        <i class="mdi mdi-delete"></i>
                                                    </a>
                                                    <form id="delete-form-{{ $item->distributor_id }}"
                                                        action="{{ route('distributor.destroy', Crypt::encryptString($item->distributor_id)) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
